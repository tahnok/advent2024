...
def step(topo: dict[tuple[int,int], int], height: int, x: int, y: int) -> set[tuple[int,int]]:
    if height == 9:
        return set([(x,y)])
    next_height = height + 1
    found = set()
    if topo.get((x - 1, y), -1) == next_height:
        found |= step(topo, next_height, x-1,y)
    if topo.get((x + 1, y), -1) == next_height:
        found |= step(topo, next_height, x+1,y)
    if topo.get((x, y-1), -1) == next_height:
        found |= step(topo, next_height, x,y-1)
    if topo.get((x, y+1), -1) == next_height:
        found |= step(topo, next_height, x,y+1)
